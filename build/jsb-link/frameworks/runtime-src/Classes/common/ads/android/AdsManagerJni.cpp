...
bool AdsManagerJni::isPreloaded(int adsTye){
 ...
 void AdsManagerJni::setLayout(int adsType,int layout){
    if (!stSFJava)
    {
        LOGE("AdsManagerJni::isPreloaded() failed!");
        return ;
     ...
     JNIEnv* lEnv = JNIHelper::getJNIEnv();
     lEnv->CallVoidMethod(stSFJava, MethodSetLayout,adsType,layout);
 }

 void AdsManagerJni::destroy(){
     if (!stSFJava)
    {
        LOGE("AdsManagerJni::destroy() failed!");
        return ;
    }
     JNIEnv* lEnv = JNIHelper::getJNIEnv();
     lEnv->CallVoidMethod(stSFJava, MethodDestroy);
 }
...
bool AdsManagerJni::init(JNIEnv * pEnv, jobject pJava){
 ...
 MethodSetLayout = pEnv->GetMethodID(ClassSF, "setLayout", "(II)V");
    if (!MethodSetLayout)
    {
        LOGE("get MethodSetLayout class Failed!");
        return false;
    }


    MethodDestroy = pEnv->GetMethodID(ClassSF, "destroy", "()V");
    if (!MethodDestroy)
    {
        LOGE("get Method destroy id Failed!");
        return false;
    }
