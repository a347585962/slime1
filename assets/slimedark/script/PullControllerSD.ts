...
export default class PullController extends cc.Component {
    ...
    start() {
        let colorName = DataConfig.getInstance().getColor();
       
        this.colorName = colorName;
        cc.loader.loadRes(`slimedark/image/${colorName}`, cc.SpriteFrame, function (err, spriteFrame) {
            this.node.getChildByName('slime').getComponent(cc.Sprite).spriteFrame = spriteFrame;
        }.bind(this));
        cc.loader.loadRes(`slimedark/image/motion1_1l1_${colorName}`, cc.SpriteFrame, function (err, spriteFrame) {
            cc.find('Canvas/left_hand1/shadow').getComponent(cc.Sprite).spriteFrame = spriteFrame;
        }.bind(this));
        cc.loader.loadRes(`slimedark/image/motion1_1r1_${colorName}`, cc.SpriteFrame, function (err, spriteFrame) {
            cc.find('Canvas/right_hand1/shadow').getComponent(cc.Sprite).spriteFrame = spriteFrame;
        }.bind(this));
         ...
         this.node.getChildByName('slime').getComponent(MoveIn).actionCallBack = function () {
             ...
             cc.find('Canvas/dragon').runAction(cc.sequence(cc.fadeTo(2, 255),
                 cc.callFunc(function () {
                     this.node.getChildByName('slime').active = false;
                    CocosHelper.findNode(cc.Canvas.instance.node, "left_hand0").active = true;
                    CocosHelper.findNode(cc.Canvas.instance.node, "right_hand0").active = true;
                    CocosHelper.findNode(cc.Canvas.instance.node, "left_hand1").active = true;
                    CocosHelper.findNode(cc.Canvas.instance.node, "right_hand1").active = true;
                    CocosHelper.findNode(cc.Canvas.instance.node, "arrow_right").active = true;
                    CocosHelper.findNode(cc.Canvas.instance.node, "arrow_left").active = true;
     ...
     dynamicCreate () {
         ...
         cc.loader.loadRes(`slimedark/dragon/slime_${this.colorName}_ske`, dragonBones.DragonBonesAsset, (err, res) => {
           
            if (err) cc.error(err);
            cc.loader.setAutoReleaseRecursively(res, true);
            _armatureDisplay.dragonAsset = res;
            cc.loader.loadRes(`slimedark/dragon/slime_${this.colorName}_tex`, dragonBones.DragonBonesAtlasAsset, (err, res) => {
     ...
     addHandCm(){
         ...
         cc.find('Canvas').on('Pulling', function (arg1, arg2, arg3) {
             ...
             self.showParticle();
             if(self._loopSound == -1 && self.lachang){
    ...
    touchBackBtn() {
        TransitionScene.changeScene('fuseSlime');
    }
