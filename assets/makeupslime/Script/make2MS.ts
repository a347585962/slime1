    ...
    dealNext(){
        ...
        if(this.step == 4){
            ...
            moveInCm.actionCallBack = ()=>{
                ...
                spoon.on(cc.Node.EventType.TOUCH_START,this.spoonTouch,this);
                spoon.on(cc.Node.EventType.TOUCH_MOVE,this.spoonMoving,this);
                spoon.on(cc.Node.EventType.TOUCH_END,this.spoonEnd,this);
                spoon.on(cc.Node.EventType.TOUCH_CANCEL,this.spoonEnd,this);
            };
            
        }

    }
    spoonTouch(event: cc.Event.EventTouch){



    }
    spoonEnd(event: cc.Event.EventTouch){

        let bowl_in = CocosHelper.findNode(cc.Canvas.instance.node, "bowl_in");
        console.log();
        bowl_in.getComponent(MixComponent).stopMix();;

    }
    spoonMoving(event: cc.Event.EventTouch){
        let bowl_in = CocosHelper.findNode(cc.Canvas.instance.node, "bowl_in");
        console.log();
        bowl_in.getComponent(MixComponent).startMix();
        let bowl = CocosHelper.findNode(cc.Canvas.instance.node, "bowl");
        let spoon = bowl.getChildByName("spoon");
        // spoon.position = spoon.position.add(event.getDelta());
        spoon.setPosition(spoon.position.add(event.getDelta()));

        let pos = spoon.position;

        if(pos.x < -111){
            spoon.setPosition(cc.v2(-111, pos.y));
        }
        if(pos.x > 164){
            spoon.setPosition(cc.v2(164, pos.y));
        }
        
        if(pos.y < 137){
            spoon.setPosition(cc.v2(pos.x, 137));
        }
        if(pos.y > 271){
            spoon.setPosition(cc.v2(pos.x, 271));
        }


    }
    MIXING(event:cc.Event.EventTouch, drag:SpriteDrag, lisenter:DragEventListener, data:string){

        let moveNode =  drag.moveNode;
        drag.enabled = false;

        console.log(moveNode.x);
