...
export default class PullController extends cc.Component {
    ...
    start() {
     ...
     dynamicCreate () {
        ...
        let pos0 = left_hand0.position;
        left_hand0.position = cc.v2(-600, 0);

        let pos1 = left_hand1.position;
        left_hand1.position = cc.v2(-600, 0);

        let pos2 = right_hand0.position;
        right_hand0.position = cc.v2(600, 0);

        let pos3 = right_hand1.position;
        right_hand1.position = cc.v2(600, 0);

        left_hand0.runAction(cc.moveTo(1.0, pos0));
        left_hand1.runAction(cc.moveTo(1.0, pos1));
        right_hand0.runAction(cc.moveTo(1.0, pos2));
        right_hand1.runAction(cc.moveTo(1.0, pos3));
