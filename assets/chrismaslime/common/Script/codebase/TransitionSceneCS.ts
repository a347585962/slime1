...
import ColorRect = require('./utils/ColorRectCS');
import { CocosHelper } from './utils/CocosHelperCS';
import ShaderHelper from '../../../components/ShaderHelperCS';
import ShaderTime from '../../../components/ShaderTimeCS';
...
export default class TransitionScene extends cc.Component {
    ...
    static changeScene(newSceneName: string, prefabUrl: string = "codebase/TransitionScen") {
        ...
        return;
        cc.loader.loadRes('prefab/mask', cc.Prefab, function (err, prefab) {
            let node = cc.instantiate(prefab);
            cc.director.getScene().addChild(node);
            node.setPosition(cc.v2(cc.view.getVisibleSize().width / 2, cc.view.getVisibleSize().height / 2));
            node.opacity = 0;
            cc.game.addPersistRootNode(node);
            cc.director.preloadScene(newSceneName);
            CocosHelper.captureNodeSize(cc.find('Canvas'), cc.view.getVisibleSize().width, cc.view.getVisibleSize().height).then((texture:cc.RenderTexture)=>{
                if(texture == null){
                    cc.log('err');
                } else {
                    ...
                    console.log(cc.view.getVisibleSize().height);
                    node.getComponent(ShaderHelper).enabled = true;
                    node.getComponent(ShaderTime).enabled = true;
