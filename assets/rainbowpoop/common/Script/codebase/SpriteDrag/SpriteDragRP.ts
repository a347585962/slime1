import { DragUtil, TweenType } from "./DragUtilRP";
import DragEventListener, { DragEventType } from "./DragEventListenerRP";
import NodeComp = require("../utils/NodeCompRP");
...
@executionOrder(1)
@disallowMultiple()
@menu("common/SpriteDragRP")
@executeInEditMode()
@requireComponent(NodeComp)
export default class SpriteDrag extends cc.Component {
   ...
   hitTest(point,listener){
    ...
    private initDataInEDITOR(){
        ...
        if(this.dragRotate == null){
            this.dragRotate = -this.node.angle;
        }
    }
    ...
    private getWordPos(cameraPos:cc.Vec2):cc.Vec2 {
        ...
        if(this.node != null){
            ...
            if(_camr != null){
                return _camr.getScreenToWorldPoint(cameraPos,wordPos);
            }
        }
        return cameraPos;
   }
   ...
   onDestroy(){
    ...
    update(dt:number){
        if(this.isDraging== true &&this.isTouch == true){
            ...
            let preRotate = -this.moveNode.angle;
