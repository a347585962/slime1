...
export default class MixComponent extends cc.Component {
    ...
    _count = 0;
    mixEnable:boolean = false;
    currntTime:number = 0;
    _changeNodes:cc.Node[] = [];
    current:number = -1;
    ...
    startMix() {
        ...
        if(this._changeNodes.length == 0){
            for(let f of this.mixPaths){
                ...
                if (this.noTrim) {
                    sp.trim = false;
                    _node.height = this.node.height;
                    _node.width = this.node.width;
                }
