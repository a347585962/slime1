import AdsManager, { ADS_TYPE } from "./AdsManagerHC";
...
export default class RewardManager {
    ...
    jsonPath:string = "chocolateslime/content.json";
    //加载配置文件
    public loadConfig(){
        let self = this;
    ...
    //显示广告
    private showRewardAdsByItem(item:RewardInfoItem){
        let self = this;
        //检查网络，系统功能，稍后实现JSB绑定
    ...
    public initLisenter(){
        ...
        AdsManager.getInstance().onAdsRewarded = function(error : string, num:number, isSkip : boolean){
            //unRegisterAdsEvents();
            self.unLisenter()
            console.log(" RewardManager====>onAdsRewarded-------" + self._waitingUnLockItemInfo.getKey());
            if(isSkip)
            {
                if(self.removeRewardLoadingCall)
                {
