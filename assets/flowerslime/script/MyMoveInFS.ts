...
@ccclass
export default class MyMoveIn extends cc.Component {
            ...
            case  ActionType.Rote:
                {
                    ...
                    if(this.direction == ShowDirection.left)
                        rotate = 10;
                    action = cc.sequence(cc.spawn(cc.moveTo(time, endPosiont).easing(cc.easeBackOut()), cc.rotateBy(time * 0.58, rotate)),cc.rotateBy(time*0.58, -rotate).easing(cc.easeBackInOut()));
                }
                break;
            default:
                action = cc.moveTo(time, endPosiont).easing(cc.easeElasticOut(2.0));
