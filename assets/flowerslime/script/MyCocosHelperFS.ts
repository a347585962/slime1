...
export class MyCocosHelper {
    ...
    static findNode(_root: cc.Node = cc.director.getScene(), name: string): cc.Node {
        return MyCocosHelper.findNode_if(_root, function (_node: cc.Node) {
            return _node.name == name;
        });
    }

    static visitNode(_root: cc.Node, _visitFun: Function) {
        MyCocosHelper.findNode_if(_root, function (_node: cc.Node) {
            _visitFun(_node);
            return false;
        });
    }
    ...
    static getScreenPos(_node: cc.Node, _dir: CocosHelper.ShowDirection): cc.Vec2 {
        ...
        let topLeft = new cc.Vec3(0, _size.height, 0);
        let bottomRight = new cc.Vec3(_size.width, 0, 0);
        ...
        if (_mainC != null) {
            topLeft = _mainC.getScreenToWorldPoint(topLeft, new cc.Vec2());
            bottomRight = _mainC.getScreenToWorldPoint(bottomRight, new cc.Vec2());
        }
    ...
    static hideNode(_node: cc.Node, _dir: CocosHelper.ShowDirection, endFunc: Function = null, isAutoRemove: boolean = true, _time: number = 0.6) {
        let endPosiont: cc.Vec2 = MyCocosHelper.getScreenPos(_node, _dir);
    ...
    static showBackOut(_node: cc.Node, _dir: CocosHelper.ShowDirection, endFunc: Function = null, _time: number = 1.0) {
        ...
        let statPos: cc.Vec2 = MyCocosHelper.getScreenPos(_node, _dir);
...
*/
    ...
    static captureNode2(distNode: cc.Node) {
        ...
        let height = Math.floor(distNode.height * distNode.scaleY);
    ...
    static captureNodeSize(distNode: cc.Node, width: number, height: number) {
        return new Promise((resolve, reject) => {
            ...
            cameraNode.setPosition(cameraNode.parent.convertToNodeSpaceAR(cc.Canvas.instance.node.convertToWorldSpaceAR(cc.v2(0, 0))));
            let preGroug = distNode.group;
            distNode.group = "captureLayer";
            // 设置你想要的截图内容的 cullingMask
            camera.cullingMask = distNode["_cullingMask"];//0xffffffff;
            ...
            let texture = new cc.RenderTexture();
            // 初始化纹理大小，如果截图内容中不包含 Mask 组件，可以不用传递第三个参数
            texture.initWithSize(width, height, cc.game._renderContext.STENCIL_INDEX8);//cc.gfx.RB_FMT_S8   cc.game._renderContext.STENCIL_INDEX8
            camera.targetTexture = texture;
    ...
    static saveImageByNode(distNode: cc.Node, name: string, callback:Function) {
        this.captureNodeSize(distNode, cc.visibleRect.width, cc.visibleRect.height).then((texture: cc.RenderTexture) => {
            ...
            if (CC_JSB && !CC_PREVIEW) {
                ...
                if (success) {
                    ...
                    console.log("save image data failed, file: " + filePath);
                }
            }
    ...
    static saveImageByTexture(texture: cc.RenderTexture, name: string, callback: Function) {
        let picData = texture.readPixels();
        let width = texture.width;
        let height = texture.height;
        // let picData = CocosHelper.filpYImage(data,width,height);
        ...
        return new Promise((resolve, reject) => {
            if (CC_JSB && !CC_PREVIEW) {
                ...
                cc.loader.load(filePath, function (err, tex) {
                    if (err) {
                        console.log('sadesffa' + err);
                    } else {
                        console.log('sadesffa' + filePath);
                        resolve(tex);
                    }
                });
            }
