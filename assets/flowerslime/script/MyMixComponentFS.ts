...
@ccclass
export default class MyMixComponent extends cc.Component {
    ...
    public _count = 0;
    public mixEnable:boolean = false;
    public stopEnable:boolean = false;
    ...
    stopMix(){
        ...
        EventListener.emitEvents(MyMixComponent.MIXSTOP,this.mixLis,this);
    }
    ...
    updateMix(dt){
        ...
        EventListener.emitEvents(MyMixComponent.MIXING,this.mixLis,this);
        if(this._count>=this._changeNodes.length){
            //停止音效
            this.stopEnable = true;
            cc.audioEngine.stop(this.current);
            EventListener.emitEvents(MyMixComponent.MIXEND,this.mixLis,this);
        }
    }
