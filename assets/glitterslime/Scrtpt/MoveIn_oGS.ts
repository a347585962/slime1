    ...
    getScreenPos(_node:cc.Node,_dir:number):cc.Vec2{
        ...
        switch (_dir) {
            ...
            case  ShowDirection.top:
                endPosiont =  new cc.Vec2(curentPosition.x, topLeft.y + 500 +_node.getContentSize().width*(_node.getAnchorPoint().y));
                break;
        }
            ...
            case  ActionType.Rote:
                {
                    ...
                    action = cc.sequence(cc.spawn(cc.moveTo(time, endPosiont).easing(cc.easeBackOut()), cc.rotateBy(time * 0.58, rotate), cc.rotateBy(time*0.58, -rotate).easing(cc.easeBackInOut())), cc.rotateTo(0.2, 0));
                }
                break;
            default:
                action = cc.moveTo(time, endPosiont).easing(cc.easeElasticOut(2.0));
